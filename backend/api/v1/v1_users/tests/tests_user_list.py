...
class UserListTestCase(TestCase, ProfileTestHelperMixin):
    ...
    def test_get_user_list_by_descendants(self):
        # Get a parent administration with level > 0 (not national level)
        # since level 0 administrations are excluded from descendants filtering
        parent_admin = Administration.objects.filter(level__level=1).first()

        if not parent_admin:
            self.skipTest("No level 1 administration found for testing")

        # Get a child administration of the parent
        child_admin = Administration.objects.filter(
            parent=parent_admin
        ).first()

        if child_admin:
            # Create a user assigned to child administration
            child_user = self.create_user(
                email="child_admin@example.com",
                role_level=self.IS_ADMIN,
                administration=child_admin,
            )

            # Create a user assigned directly to parent administration
            parent_user = self.create_user(
                email="parent_admin@example.com",
                role_level=self.IS_ADMIN,
                administration=parent_admin,
            )

            # Test with descendants=true (default)
            response = self.client.get(
                f"/api/v1/users?page=1&administration={parent_admin.id}"
                "&descendants=true",
                HTTP_AUTHORIZATION=f"Bearer {self.token}",
                content_type="application/json",
            )
            ...
            # Should find users from both parent and child administrations
            found_child_user = any(
                user["email"] == child_user.email for user in user_data["data"]
            )
            found_parent_user = any(
                user["email"] == parent_user.email
                for user in user_data["data"]
            )
            self.assertTrue(found_child_user)
            self.assertTrue(found_parent_user)

            # Test with descendants=false
            response = self.client.get(
                f"/api/v1/users?page=1&administration={parent_admin.id}"
                "&descendants=false",
                HTTP_AUTHORIZATION=f"Bearer {self.token}",
            ...
            # Should NOT find users from child administrations when
            # descendants=false. Only users directly assigned to the
            # parent administration should be found
            found_child_user = any(
                user["email"] == child_user.email for user in user_data["data"]
            )
            found_parent_user = any(
            ...
            self.assertFalse(found_child_user)
            self.assertTrue(found_parent_user)
        else:
            # Skip test if no child administration exists
            self.skipTest("No child administration found for testing")

    def test_get_user_list_level_zero_admin_no_descendants_filter(self):
        # Test that level 0 (national level) administrations are excluded
        # from descendants filtering - they should return all users
        national_admin = Administration.objects.filter(level__level=0).first()

        if not national_admin:
            self.skipTest("No level 0 administration found for testing")

        # Create test users in different administrations
        test_user = SystemUser.objects.create(
        ...
        # Test with level 0 administration - should ignore descendants filter
        response = self.client.get(
            f"/api/v1/users?page=1&administration={national_admin.id}"
            "&descendants=false",
            HTTP_AUTHORIZATION=f"Bearer {self.token}",
            content_type="application/json",
        )
        ...
        # Since level 0 is excluded from filtering, it should return all users
        # including our test user (no administration filter applied)
        found_test_user = any(
            user["email"] == test_user.email for user in user_data["data"]
        )
        self.assertTrue(found_test_user)
